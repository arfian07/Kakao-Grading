...
import { formatDate, formatRupiah } from "../lib/mockData";
import {
    Search,
    Eye,
    X,
        ...
        <div className="h-full flex flex-col p-2.5 gap-2.5" data-testid="history-page">
            ...
            <div className="grid grid-cols-4 gap-2">
                ...
                <div className="card-grad rounded-md border border-border px-2.5 py-1.5">
                    ...
                    <div className="num-display text-base font-bold">
                        ...
                        <span className="text-[9px] text-muted-foreground ml-1 font-sans">
                            kg
                        </span>
                    </div>
                </div>
                <div className="card-grad rounded-md border border-border px-2.5 py-1.5">
                    ...
                    <div className="num-display text-base font-bold text-accent">
                ...
                <div className="card-grad rounded-md border border-border px-2.5 py-1.5 flex items-center gap-1.5">
                    ...
                    <div>
                        ...
                        <div className="num-display text-base font-bold text-emerald-400">
                            {filtered.length > 0
                                ? Math.round(
                                      (filtered.filter((t) => t.grade === "Mutu I").length / filtered.length) * 100
                                  )
                                : 0}
                            %
                        </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <button
                    ...
                    className="h-8 px-2.5 rounded-md bg-muted/60 hover:bg-muted border border-border text-[10px] font-medium flex items-center gap-1.5"
                >
            ...
            <div className="flex-1 min-h-0 rounded-lg border border-border card-grad overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar" data-testid="history-table">
                    ...
                    {filtered.map((t, i) => (
                        ...
                        >
                            <div className="font-mono text-accent">{t.id}</div>
                            <div className="text-muted-foreground">
                                {formatDate(t.date)}
                            </div>
                            <div className="num-display text-right font-semibold">
                                {t.weight.toFixed(1)}g
                            </div>
                            <div className="num-display text-right">
                                {t.beanCount}
                            </div>
                            <div>
                                <span
                                    className={`px-1.5 py-0.5 rounded text-[9px] font-semibold border ${gradeStyles[t.grade]}`}
                                >
                                    {t.grade}
                                </span>
                            </div>
            ...
            {detail && (
                <div
                    className="absolute inset-0 z-30 flex items-center justify-center p-3 bg-background/80 backdrop-blur-sm"
                    onClick={() => setDetail(null)}
                    data-testid="trx-detail-modal"
                >
                    <div
                        onClick={(e) => e.stopPropagation()}
                        className="w-[640px] max-h-full card-grad border border-border rounded-lg overflow-hidden shadow-2xl"
                    >
                        <div className="flex items-center justify-between px-3 h-9 border-b border-border">
                            <div className="flex items-center gap-2">
                                ...
                                <span
                                    className={`px-1.5 py-0.5 rounded text-[9px] font-semibold border ${gradeStyles[detail.grade]}`}
                                >
                                    {detail.grade}
                                </span>
                            </div>
                        ...
                        <div className="grid grid-cols-[260px_1fr] gap-3 p-3">
                            <div
                                className="rounded-md overflow-hidden border border-border aspect-[4/3] relative bg-black"
                                style={{
                                    background:
                            ...
                            <div className="space-y-2 text-[11px]">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="rounded-md border border-amber-400/30 bg-amber-500/10 px-2 py-1">
                                        <div className="text-[8px] uppercase text-amber-400">
                                            Berjamur
                                        </div>
                                        <div className="num-display font-bold">
                                            {detail.moldy}
                                    ...
                                    <div className="rounded-md border border-secondary/30 bg-secondary/10 px-2 py-1">
                                        <div className="text-[8px] uppercase text-red-300">
                                            Hitam
                                        </div>
                                        <div className="num-display font-bold">
                                            {detail.black}
                                    ...
                                    <div className="rounded-md border border-destructive/30 bg-destructive/10 px-2 py-1">
                                        <div className="text-[8px] uppercase text-destructive">
                                            Cacat
                                        </div>
                                        <div className="num-display font-bold">
                                            {detail.defective}
